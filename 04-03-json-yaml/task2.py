...
ip_cache_filename = 'services_ip'

class IpCache:
    def __init__(self, services, filename):
        self.__services = services
        self.__filename = filename
        self.__filename_csv = filename + '.csv'
        self.__ip_cache = {}
        self.read()

    def read(self):
        if not os.path.isfile(self.__filename_csv):
            return
        with open(self.__filename_csv) as f:
            reader = csv.reader(f)
            for row in reader:
    ...
    def write(self):
        with open(self.__filename_csv, 'w') as f:
            writer = csv.writer(f)
            for service, ip in self.__ip_cache.items():
                writer.writerow([service, ip])

        services_list = [{service: ip} for service, ip in self.__ip_cache.items()]

        with open(self.__filename + '.json', 'w') as f:
            f.write(json.dumps(services_list))

        with open(self.__filename + '.yaml', 'w') as f:
